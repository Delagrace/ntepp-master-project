...
with open('C:/Users/delag/OneDrive/Documents/blockchain-network-on-kubernetes-correct/node-red1/flows-registration.json', 'r') as file:
...
def update_sensor_id(node, start, end):
    for rule in node['rules']:
        ...
        if 'args' in to_dict:
            sensor_id = to_dict['args']['sensorID']
            if sensor_id.startswith('sensor'):
                current_sensor_number = int(sensor_id.split('sensor')[-1])
                if start <= current_sensor_number <= end:
                    new_sensor_number = current_sensor_number + 250
                    to_dict['args']['sensorID'] = 'sensor' + str(new_sensor_number)
