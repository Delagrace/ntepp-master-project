...
const { FileSystemWallet, Gateway, X509WalletMixin, DefaultEventHandlerStrategies } = require('fabric-network');
const fs = require('fs');
...
class FabricService {
    ...
    async registerSensor(username, channel, smartcontract, args) {
        try {
            ...
            contract.addContractListener('registerSensorListener', 'registerSensor', (err, event, blockNumber, transactionId, status) => {
                if (err) {
                    console.error(err);
                    return;
                }
                console.log(`Transaction ${transactionId} in block ${blockNumber} was ${status}`);
    ...
    async addTemp(username, channel, smartcontract, args) {
        try {
            ...
            contract.addContractListener('addTempListener', 'addTemp', (err, event, blockNumber, transactionId, status) => {
                if (err) {
                    console.error(err);
                    return;
                }
